...
export const templateForgotPassword = (email: string, forgot_password_token: string): MailTemplate => {
  return {
    from: `${env.EMAIL_USER}`,
    to: `${email}`,
    ...
    html: `<body style="font-family: Arial, sans-serif;
            ...
            <div class="email-container" style="max-width: 600px;
                ...
                <div class="email-content" style="padding: 20px">
                    <p style= "color: black; line-height: 1.6">Hi ${email},<br>
                    We received a request to reset your password for your [Booking event web] account.<br>
                    ...
                    <button style="
                      ...
                      text-decoration: none;
                      transition: all 0.2s ease;">
                      <a style="text-decoration: none; color: white" href="${env.DB_HOST}:${env.PORT}/users/verify-forgot-password-token?forgot_password_token=${forgot_password_token}">Reset Password</a>
                    </button>
                </div>
            </div>
        </body>
